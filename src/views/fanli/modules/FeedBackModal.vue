    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="反馈内容">
          <a-input
            placeholder="请输入反馈内容"
            v-decorator="['feedbackContent', validatorRules.feedbackContent ]"
          />
        </a-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      validatorRules: {
        feedbackContent: { rules: [{ required: true, message: '请输入反馈内容!' }] },
  ...
  methods: {
    ...
    edit(record) {
      ...
      this.visible = true
      this.$nextTick(() => {
        this.form.setFieldsValue(pick(this.model, 'feedbackContent', 'vipId'))
        //时间格式化
      })
    },
    close() {
